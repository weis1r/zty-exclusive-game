...
Task
- 用户新需求: 当前游戏没有针对手机端进行适配，需要优化移动端布局，并在完成后推送远程分支和重新部署。

What changed
- `src/App.tsx`
  - 章节预览卡改成可点击切换的按钮。
  - 新增 `focusChapter(chapterId)`，手机端点击章节卡后会把当前焦点切到该章节的首个可进入关卡。
  - 章节列表增加 `chapter-section--active` 状态，方便移动端只展开当前章节，减少超长页面。
- `src/App.css`
  - 首页移动端改成“章节横向预览 + 当前章节展开”的结构，避免 20 关在窄屏上一口气全摊开。
  - 手机端顶部按钮改成双列，工具按钮改成 2x2，状态条改成 2 列，减少竖向拉长。
  - 收集槽砖块尺寸从固定宽度改成随网格缩放，解决窄屏下 7 槽横向溢出。
  - 压缩了移动端的面板 padding、棋盘外框、弹窗宽度和局内标题字号，让棋盘和操作区更平衡。
  - 增加了 `safe-area` padding 兜底，兼容刘海屏/底部手势区。
...
- 自动化检查：
  ...
  - `npm run build`
- 浏览器移动端验收：
  - 390x844 首页截图：当前章节突出，其余章节以横向预览卡展示，页面长度明显缩短。
  - 390x844 第 2 关截图：目标卡、状态卡、工具区、棋盘、收集槽都在手机宽度内正常显示。
  - 点击第 2 章预览卡后，页面会切换到第 2 章焦点和第 2 章关卡区，说明章节切换正常。
